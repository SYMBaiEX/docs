...
The agent status is included in the main agent details endpoint. To check if an agent is active, use the GET endpoint and check the `status` field.
...
This returns all agents with their current status (active/inactive) based on whether they have a running runtime.
...
Agent logs are available through the Server API and WebSocket streaming. Individual agent logs can be filtered through the centralized logging system.
...
For detailed log streaming implementation, see the [WebSocket Events documentation](../core-concepts/communication/websocket-events).
...
Room management is handled through the Memory API. For agent-specific room operations, use the Memory API endpoints.
...
*Note: Panel functionality is currently under development.*
