...
The Media API provides comprehensive file upload and media handling capabilities for agents and channels. All media endpoints are prefixed with `/api/media`.
...
*Note: The Media API is designed to be extensible. Additional processing capabilities and storage backends can be added through plugins.*