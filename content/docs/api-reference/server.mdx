...
The Server API provides runtime health monitoring, system control, and debugging capabilities for the ElizaOS server. All server endpoints are prefixed with `/api/server`.
...
*Note: Debug and control endpoints require proper authentication in production environments.*