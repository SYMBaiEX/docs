---
title: Core Actions
description: Complete reference for the 12 essential bootstrap actions that provide foundational functionality for ElizaOS agents
---

# Core Actions Reference

This document provides comprehensive documentation for the 12 core bootstrap actions that form the foundation of ElizaOS agent functionality. These actions are essential for agent operation and are automatically included in the bootstrap plugin.
...
**Description:** Start following a channel with great interest, allowing the agent to chime in without needing explicit mentions.

### Properties

- **Similes:** `FOLLOW_CHAT`, `FOLLOW_CHANNEL`, `FOLLOW_CONVERSATION`, `FOLLOW_THREAD`
- **Validation:** Checks for follow-related keywords and ensures room is not already followed or muted
...
**Description:** Ignore the user in a conversation when they are aggressive, creepy, or when the conversation has naturally ended.
...
export const ignoreAction: Action = {
  name: 'IGNORE',
  similes: ['STOP_TALKING', 'STOP_CHATTING', 'STOP_CONVERSATION'],
  description: 'Call this action if ignoring the user. Use IGNORE when the user is aggressive, creepy, or the conversation has naturally ended.',
...
**Description:** Mute a room, ignoring all messages unless explicitly mentioned. Use when asked to stop responding or when being disruptive.

### Properties

- **Similes:** `MUTE_CHAT`, `MUTE_CONVERSATION`, `MUTE_ROOM`, `MUTE_THREAD`, `MUTE_CHANNEL`
- **Validation:** Ensures room is not already muted
- **Handler:** Uses LLM to determine if muting is appropriate, then sets participant state to 'MUTED'
...
**Description:** Respond but perform no additional action. This is the default behavior when the agent is speaking without taking any specific action.
...
**Description:** Generate and send a reply to the current conversation using the LLM. This is the primary response mechanism for agents.
...
**Description:** Assigns a role (Admin, Owner, None) to a user or list of users in a channel. Requires proper permissions and world context.
...
**Description:** Send a message to a user or room other than the current one. Supports cross-platform messaging.
...
**Description:** Save configuration settings during onboarding or update existing settings. Used for world/server configuration.
...
export async function getWorldSettings(runtime: IAgentRuntime, serverId: string): Promise<WorldSettings | null> {
...
export async function updateWorldSettings(runtime: IAgentRuntime, serverId: string, worldSettings: WorldSettings): Promise<boolean> {
  const worldId = createUniqueUuid(runtime, serverId);
  const world = await runtime.getWorld(worldId);
...
**Description:** Stop following a channel. Agent can still respond when explicitly mentioned but won't automatically chime in.
...
export const unfollowRoomAction: Action = {
  name: 'UNFOLLOW_ROOM',
  similes: ['UNFOLLOW_CHAT', 'UNFOLLOW_CONVERSATION', 'UNFOLLOW_ROOM', 'UNFOLLOW_THREAD'],
  description: "Stop following this channel. You can still respond if explicitly mentioned, but you won't automatically chime in anymore.",
...
**Description:** Unmute a room, allowing the agent to consider responding to messages again after being muted.
...
**Description:** Add or edit contact details for a person in the conversation. Used for relating entities across platforms.

### Properties

- **Similes:** `UPDATE_ENTITY`
- **Validation:** Always returns true (core functionality)
- **Handler:** Uses LLM to extract platform and contact information, then creates/updates entity components
...
export const updateEntityAction: Action = {
  name: 'UPDATE_CONTACT',
  similes: ['UPDATE_ENTITY'],
  description: 'Add or edit contact details for a person you are talking to or observing in the conversation. Use this when you learn this information from the conversation about a contact.',
...
This comprehensive reference covers all 12 core bootstrap actions essential for ElizaOS agent operation. Each action is designed to handle specific aspects of agent behavior while maintaining consistency and reliability across the platform.