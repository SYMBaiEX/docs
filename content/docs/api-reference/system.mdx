...
The System API provides environment configuration and version information for the ElizaOS server. All system endpoints are prefixed with `/api/system`.
...
*Note: System API endpoints provide essential information for client applications to adapt their behavior based on server capabilities and configuration.*