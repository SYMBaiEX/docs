...
The bootstrap plugin provides essential providers that form the foundation of agent functionality in ElizaOS. These providers supply crucial data and context to agents during runtime, enabling them to understand their environment, access conversation history, manage state, and respond appropriately.
...
Provides information about available actions that the agent can perform based on the current context.
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    ...
    return {
      ...
      values: {
        ...
        actionsWithDescriptions: actionsData.map(action => 
...
Provides behavioral guidance to help the agent respond appropriately based on the communication channel type (group, DM, voice, etc.).
...
Provides information about the agent's capabilities by collecting descriptions from all registered services.
...
export const characterProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    ...
    const bioText = Array.isArray(character.bio)
      ? character.bio.sort(() => 0.5 - Math.random()).slice(0, 10).join(' ')
      : character.bio || '';

    // Random topic and adjective selection
    const topicString = character.topics?.length > 0 
      ? character.topics[Math.floor(Math.random() * character.topics.length)]
      : null;
      
    const adjectiveString = character.adjectives?.length > 0
...
export const entitiesProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory) => {
    ...
    const entities = formattedEntities && formattedEntities.length > 0
...
export const factsProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, _state?: State) => {
    ...
    const allFacts = [...relevantFacts, ...recentFactsData]
      .filter((fact, index, self) => 
        index === self.findIndex(t => t.id === fact.id)
      );

    const formattedFacts = allFacts.reverse()
      .map(fact => fact.content.text).join('\n');
...
export const recentMessagesProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory) => {
    ...
    const [formattedRecentMessages, formattedRecentPosts] = await Promise.all([
      ...
      formatPosts({ messages: recentMessagesData, entities: entitiesData, conversationHeader: false })
    ]);
...
export const relationshipsProvider: Provider = {
  name: 'RELATIONSHIPS',
  description: 'Relationships between the agent and other people, or between other people that the agent has observed interacting with',
  dynamic: true,
  get: async (runtime: IAgentRuntime, message: Memory) => {
...
export const roleProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State): Promise<ProviderResult> => {
    ...
    if (!room || room.type !== ChannelType.GROUP) {
      return {
        data: { roles: [] },
        values: { roles: 'No access to role information in DMs, the role provider is only available in group scenarios.' },
    ...
    for (const entityId of Object.keys(roles)) {
      ...
      if (user?.metadata?.name && user?.metadata?.username) {
        ...
        switch (userRole) {
          case 'OWNER': owners.push(userInfo); break;
          case 'ADMIN': admins.push(userInfo); break;
          default: members.push(userInfo); break;
        }
      }
    }
...
export const worldProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory) => {
    ...
    const channelsByType = {
      text: [], voice: [], dm: [], feed: [], thread: [], other: []
    };
    ...
    worldRooms.forEach(room => {
      ...
      if (room.type === ChannelType.GROUP || room.type === ChannelType.WORLD || room.type === ChannelType.FORUM) {
...
This comprehensive documentation covers all core bootstrap providers, their functionality, usage patterns, and integration examples. Each provider serves a specific purpose in the agent's ecosystem, from managing conversation state to providing environmental context and behavioral guidance.