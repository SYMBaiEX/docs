...
- **AgentRuntime**: The core runtime environment (`@elizaos/core`) that manages agent lifecycle, memory, and execution
- **Plugin System**: Extensible architecture allowing custom actions, evaluators, and providers through standardized interfaces
- **Project Structure**: Monorepo organization with packages for core, CLI, client, server, and plugins
- **Package Organization**: Clean separation of concerns with `@elizaos/core`, `@elizaos/cli`, `@elizaos/client`, `@elizaos/server`
