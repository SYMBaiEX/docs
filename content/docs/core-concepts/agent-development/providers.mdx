---
title: "Providers"
description: "Understanding providers in ElizaOS, including the Provider interface, data retrieval patterns, and real-world examples from plugin-bootstrap for contextual information"
---
...
export const factsProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    ...
    const text = facts.length > 0 
      ? addHeader('# Known Facts', formattedFacts)
      : '';
...
export const relationshipsProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    ...
    const text = relationships.length > 0
      ? addHeader('# Entity Relationships', formattedRelationships)
      : '';
...
export const capabilitiesProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    ...
    const formattedActions = actionNames
      .map(name => `• ${name}`)
      .join('\n');
    
    const formattedEvaluators = evaluatorNames
      .map(name => `• ${name}`)
      .join('\n');
...
export const databaseProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    ...
    const text = addHeader('# Database Results', 
...
export const fileSystemProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    ...
    const text = addHeader('# File System', 
...
export const cachingProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    ...
    const text = addHeader('# Cached Data', 
...
export const streamingProvider: Provider = {
  ...
  get: async (runtime: IAgentRuntime, message: Memory, state: State) => {
    ...
    const text = addHeader('# Streaming Data', 
