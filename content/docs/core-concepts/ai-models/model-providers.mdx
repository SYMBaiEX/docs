...
description: Working with different AI model providers in elizaOS v1.2.0 with enhanced plugin-based architecture
---
...
async function generateWithFallback(runtime, prompt, requirements = {}) {
  ...
  for (const handler of sortedHandlers) {
    try {
      return await runtime.useModel(modelType, {
        prompt,
        temperature: 0.7,
        context: {
          requestId: uuidv4(),
