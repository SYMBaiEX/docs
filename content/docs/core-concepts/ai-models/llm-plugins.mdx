...
```typescript
import { Plugin, ModelType, IAgentRuntime } from '@elizaos/core';
...
```typescript
import { Plugin, ModelType, GenerateTextParams } from '@elizaos/core';
...
const character = {
  ...
  modelProvider: 'anthropic', // Primary provider
};
```
