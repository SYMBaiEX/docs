...
description: How Eliza manages agent knowledge, facts, and document processing for intelligent context retrieval
---

## Overview

Eliza's knowledge management system provides agents with the ability to store, organize, and retrieve structured knowledge. This includes facts about users, documents, and domain-specific information that can be used to enhance conversations and decision-making.
...
const factsProvider: Provider = {
  ...
  get: async (runtime, message, state) => {
    ...
    const facts = [...userFacts, ...relevantFacts]
...
const taggedKnowledge = await runtime.getMemories({
  tableName: 'facts',
  count: 50
}).then(memories => 
  memories.filter(m => 
    m.metadata?.tags?.includes('policy')
  )
);
```
...
class KnowledgeOrganizer {
  async buildHierarchy(domain: string): Promise<KnowledgeHierarchy> {
    ...
    const facts = await runtime.getMemories({
      ...
      memories.filter(m => 
        m.metadata?.tags?.includes(domain)
      )
    );
...
async function getRelatedKnowledge(factId: UUID) {
  ...
  const relatedFacts = await Promise.all(
    relationships.map(r => 
      runtime.getMemoryById(r.targetEntityId)
    )
  );
...
class DomainKnowledgeSystem {
  ...
  async getRelevantKnowledge(query: string): Promise<Memory[]> {
    ...
    return domainKnowledge.filter(k =>
      k.metadata?.tags?.includes(this.domain)
    );
  }
...
- [Character Configuration](../agents/character-files) - Defining agent knowledge