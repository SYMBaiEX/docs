...
description: Understanding Eliza's database architecture, adapters, and the Entity-Component System
---

## Overview

Eliza uses a flexible database architecture built around the Entity-Component System (ECS) pattern. This design allows for extensible data storage with support for multiple database backends through a unified adapter interface.
...
abstract class DatabaseAdapter<DB = unknown> implements IDatabaseAdapter {
  ...
  abstract getComponent(entityId: UUID, type: string): Promise<Component | null>;
  abstract createComponent(component: Component): Promise<boolean>;
  abstract updateComponent(component: Component): Promise<void>;
  
  // Memory operations
  abstract getMemories(params: MemoryParams): Promise<Memory[]>;
  abstract createMemory(memory: Memory, tableName: string): Promise<UUID>;
  abstract searchMemories(params: SearchParams): Promise<Memory[]>;
...
// Retrieve component
const userPrefs = await runtime.getComponent(
  userId,
  "preferences",
  worldId
);
...
await adapter.runMigrations([
  ...
  'packages/plugin-custom/migrations'
]);
...
async function createEntityWithComponents(
  entity: Entity,
...
class GameEntitySystem {
  ...
  async createPlayer(name: string, roomId: UUID): Promise<Entity> {
    ...
    const player = await runtime.createEntity({
      names: [name],
  ...
  async getPlayerStats(playerId: UUID): Promise<any> {
    const stats = await runtime.getComponent(playerId, "stats");
  ...
  async updatePlayerStats(
    ...
    await runtime.updateComponent(stats);
  }
}
```
...
- [Plugin Development](../development/creating-plugins) - Creating database plugins