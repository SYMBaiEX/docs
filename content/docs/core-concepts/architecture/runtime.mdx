...
description: "Deep dive into the AgentRuntime class and its lifecycle management in v1.2.0"
---

# Runtime Architecture

The ElizaOS v1.2.0 runtime architecture is built around the enhanced `AgentRuntime` class, which serves as the core orchestrator for all agent operations. This system provides a flexible, event-driven framework that manages plugins, services, models, and agent lifecycle with improved run ID tracking, service management, and enhanced plugin dependency resolution.
...
export class AgentRuntime implements IAgentRuntime {
  ...
  readonly runId: UUID; // New in v1.2.0: Enhanced run tracking
  public adapter!: IDatabaseAdapter;
  readonly actions: Action[] = [];
  readonly evaluators: Evaluator[] = [];
  readonly providers: Provider[] = [];
  readonly plugins: Plugin[] = [];
  readonly services: Map<ServiceType, Service> = new Map(); // Enhanced service management
  readonly events: Map<string, ((params: any) => Promise<void>)[]> = new Map();
  private isInitialized = false;
  private currentActionContext?: ActionContext; // New: Action context tracking
...
- **runId**: Unique identifier for the current runtime session (new in v1.2.0)
...
- **services**: Map of registered services with enhanced lifecycle management
- **events**: Event system for inter-component communication
- **currentActionContext**: Context tracking for action execution (new in v1.2.0)

## Runtime Lifecycle

### 1. Initialization

The runtime follows a structured initialization process with enhanced dependency resolution:
...
async initialize(): Promise<void> {
  ...
  this.runId = uuidv4() as UUID;

  // Resolve plugin dependencies (new in v1.2.0)
  const resolvedPlugins = await this.resolvePluginDependencies(this.characterPlugins);
  
  // Register all plugins in dependency order
  ...
  // Run plugin migrations (enhanced in v1.2.0)
...
Each plugin is registered through a comprehensive process with enhanced dependency tracking:
...
async registerPlugin(plugin: Plugin): Promise<void> {
  ...
  // Register model handlers (enhanced in v1.2.0)
  if (plugin.models) {
  ...
  // Register event handlers (new in v1.2.0)
  if (plugin.events) {
    Object.entries(plugin.events).forEach(([eventName, handlers]) => {
      handlers.forEach(handler => this.registerEvent(eventName, handler));
    });
  }
}
```
