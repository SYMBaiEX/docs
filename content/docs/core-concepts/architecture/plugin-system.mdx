---
title: "Plugin System"
description: "Complete guide to the ElizaOS plugin architecture and extension system with v1.2.0 enhancements"
---
  ...
  async handleMessage(context: any) {
    ...
    if (actionContext) {
      this.runtime.logger.debug(
        `Handling message in action context: ${actionContext.actionName}`
      );
    }
...
async function resolveDependencies(plugins: Plugin[]): Promise<Plugin[]> {
  ...
  while (pending.length > 0 && iterations < maxIterations) {
    ...
    const unresolvedDeps = plugin.dependencies?.filter(dep => {
      ...
      if (version && !isVersionCompatible(resolvedPlugin.version, version)) {
        throw new Error(`Plugin ${name} version ${resolvedPlugin.version} incompatible with required ${version}`);
      }
    ...
    if (plugin.peerDependencies) {
      for (const dep of plugin.peerDependencies) {
        ...
        if (!peerPlugin) {
          ...
          console.warn(`Peer dependency ${name} version ${peerPlugin.version} incompatible with required ${version}`);
        }
      }
    }
  ...
  if (pending.length > 0) {
    ...
    throw new Error(`Circular dependency detected or missing dependencies: ${pendingNames.join(', ')}`);
  }
