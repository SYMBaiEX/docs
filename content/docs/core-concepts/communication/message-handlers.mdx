...
Message handlers are the core processing units in ElizaOS that handle incoming messages, generate responses, and execute actions. The bootstrap plugin provides the primary message handling pipeline, orchestrating the entire agent response lifecycle from message reception to final output.
...
The `MessageBusService` is the intermediary that connects agents to the central message bus and handles message routing:
...
if (!shouldSkipShouldRespond) {
  ...
  shouldRespond = responseObject?.action && 
...
export function shouldBypassShouldRespond(
  ...
  return bypassTypes.has(roomType) || 
...
if (responseContent && message.id) {
  ...
  const isSimple = responseContent.actions?.length === 1 &&
