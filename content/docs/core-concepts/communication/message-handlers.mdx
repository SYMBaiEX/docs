...
<Callout type="info">
**Platform-Agnostic Design**: The `processMessage` function and message handling pipeline are **completely platform-agnostic**. They work identically whether messages come from Discord, Twitter, Telegram, REST API, WebSocket, or any other client. The client type only affects how messages are initially ingested into the system, not how they are processed. This unified approach ensures consistent agent behavior across all platforms.
</Callout>
...
3. **Processing**: Converts message format and emits to agent runtime (same process for all clients)
